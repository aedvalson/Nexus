	...
	<div id="bulletManageInventory" style="height:auto;" class="navContent">
		<div class="divFilters">
			<div>
				...
				<INPUT style="width:100%" id="tbNameV">
			</div>
			...
			<div>
				<b>Date Added:</b><br>
				<label>Start:</label>
				<INPUT style="width:90%" id="tbStartDateV" class="datepicker" value=""></input>

				<label>End:</label>
				<INPUT style="width:90%" id="tbEndDateV" class="datepicker" value="">
			</div>
			...
			if (args.success == "success")
			{
				...
				if (args.output)
				{
					...
					$('.deleteLink').click( function() {
						if (confirm("Are you sure you wish to permanently delete this Team? If there are currently users in this team, they will not show up correctly until their team is updated (fix later)."))
						{
							var team_id = this.id.replace("deleteLink_", "");
							$.post('/<?= $ROOTPATH ?>/Includes/ajax.php', 
								{ id: "deleteTeam",
									team_id: team_id}, function(json) {
								eval("var args = " + json);		
								if (args.success == "success")
								{
									createFilters();
