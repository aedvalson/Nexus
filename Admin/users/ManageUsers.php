       	...
       	$.post('/<?= $ROOTPATH ?>/Includes/ajax.php', filters, function(json) {
			...
			if (args.success == "success")
			{
				...
				if (args.output)
				{
					...
					{
						_r = args.output[r];
						$('#theTable').append('<tr class="row'+row+'" id="row_' + _r.user_id + '"><td><div class="selected" style="display:none" id="commandDivSelected_' + _r.user_id + '"><a href="#" id="btnSave_' + _r.user_id + '" class="btnSave">' + saveLinkContent() +'</a><a href="#" class="cancelLink">' + cancelLinkContent() + '</a></div><div class="unselected" id="commandDiv_' + _r.user_id + '">		<? if ($_SESSION["perm_level"] >= 100) { ?><a href="#" id="editLink_' + _r.user_id + '" class="editLink" >' + editLinkContent() + '</a><? } ?><? if ($_SESSION["perm_level"] >= 1000) { ?><a href="#" id="deleteLink_' + _r.user_id + '" class="deleteLink" >' + deleteLinkContent() + '</a></div><? } ?></td><td><div class="view"><span id="spnUsername_' + _r.user_id + '">'+_r.Username+'</span></div><div class="editCell"><input id="tbUsername_'+_r.user_id+'" value="'+_r.Username+'"></div></td><td><div class="view"><span id="spnFirstName_' + _r.user_id + '">'+_r.FirstName+'</span></div><div class="editCell"><input id="tbFirstName_'+_r.user_id+'" value="'+_r.FirstName+'"></div></td><td><div class="view"><span id="spnLastName_' + _r.user_id + '">'+_r.LastName+'</span></div><div class="editCell"><input id="tbLastName_'+_r.user_id+'" value="'+_r.LastName+'"></div></td><td><div class="view"><span style="display:none;" id="spanTeamId_' + _r.user_id +'">'+_r.team_id+'</span><span id="spnTeam_' + _r.user_id + '">'+_r.team_name+'</span></div><div class="editCell"><SELECT id="ddlTeam_'+_r.user_id+'"><?
							$teams = $DB->getTeams();
							foreach ($teams as $team)
							{
								?><OPTION value="<?= $team["team_id"] ?>"><? echo $team["team_name"]; ?></option><?
