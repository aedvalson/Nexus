...
<div class="reportContainer">
	<div class="reportHeader">
		<h1>General Commission Report</h1>
		<h2><?= date("m/d/Y", strtotime ( $params["startDate"] )) ?> to <?= date("m/d/Y", strtotime ( $params["endDate"] )) ?></h2>
		<h2><?= $AgencyParams["AgencyName"] ?></h2>
	</div>

	<table border="0" style="border: none;width: 1000px; font-size: 150%;" class="report">
		<tr>
			<td style="border: none; font-weight: bold;">Staff Members Earning Commissions: <?= $dealerCount ?></td>
			<td style="border: none; font-weight: bold;">Total Commissions Paid in Period: $<?= money_format("%i", $gComm) ?></td>
			<td style="border: none; font-weight: bold;">Highest Earner: <?= $highestEarner ?></td>
		</tr>
	</table>

	<br />

	<h3>General Summary</h3><br/>
	<table class="report" CELLSPACING="0" style="width: 1000px; margin-bottom: 1em">
	<tr>
		<td></td>
		<td>Commissions</td>
		<td>Bonuses</td>
	...
	?>
	<tr>
		<td><?= $users[$userHash["user_id"]]["FirstName"] ?> <?= $users[$userHash["user_id"]]["LastName"] ?></td>
		<td>$<?= money_format("%i", $commTotal) ?></td>
		<td>0</td>
		<td>$<?= money_format("%i", $adjustmentTotal ) ?></td>
	...
	<tr>
		<td></td>
		<td>$<?= money_format("%i", $gComm) ?></td>
		<td>0</td>
		<td>$<?= money_format("%i", $gAdjustment) ?></td>
		<td>$<?= money_format("%i", $gTotal) ?></td>
	</tr>
	</table>
	<h3>Detailed Summaries</h3><br/>
	...
	<span style="margin-top: 1em">Staff Member: <?= $users[$userHash["user_id"]]["FirstName"] ?> <?= $users[$userHash["user_id"]]["LastName"] ?> (<?= $users[$userHash["user_id"]]["Username"] ?>)</span>
